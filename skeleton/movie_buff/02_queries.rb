...
def vanity_projects
  ...
  .joins(:actors).where("actors.id = movies.director_id")
...
def most_supportive
  ...
  Actor.select(:id,:name,'count(*) as roles').joins(:castings).where('castings.ord != 1').group("actors.id").order('roles DESC').limit(2)
end
